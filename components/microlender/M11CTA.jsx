...
// `.deck-anim` stagger (see globals.css); the "get started" live dot uses the
// CSS `.deck-pulse`. Keeps the slide content out of any client bundle chunk.
...
export default function M11CTA() {
  return (
    <div className="deck-anim is-in" style={{ position: "absolute", inset: 0, background: "#F4EFE3", overflow: "hidden", fontFamily: '"Geist", system-ui, sans-serif' }}>
      ...
      <div
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "clamp(14px, 1.5vw, 20px)", maxWidth: "min(1000px, 86%)" }}>
          ...
          <h1 style={{ margin: 0, fontWeight: 700, fontSize: "clamp(40px, 6.8vw, 96px)", lineHeight: 1.02, letterSpacing: "-0.03em" }}>
            <span style={{ color: "#16291C" }}>Kita is AI for microlending.</span><br />
            <span style={{ color: "#2D6A3F" }}>Ready when you are.</span>
          </h1>
          <p style={{ margin: "clamp(10px,1vw,14px) 0 0", color: "#5A7060", fontSize: "clamp(15px, 1.5vw, 22px)", lineHeight: 1.45, maxWidth: "820px" }}>
          ...
          <div style={{ marginTop: "clamp(16px,1.6vw,28px)" }}>
            <a
              href="https://cal.com/carmel-limcaoco/meet-with-kita-carmel-limcaoco"
              target="_blank"
              ...
              style={{
                ...
                padding: "clamp(13px,1.2vw,18px) clamp(22px,2vw,32px)",
                borderRadius: "12px",
                textDecoration: "none",
              }}
            >
              Book a call <span aria-hidden style={{ fontSize: "1.1em" }}>→</span>
            </a>
          </div>
        </div>

        <div style={{ color: "#8E9991", fontSize: "clamp(11px, 1vw, 14px)", letterSpacing: "0.06em", lineHeight: 1.6 }}>
